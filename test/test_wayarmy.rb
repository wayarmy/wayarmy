...
class WayarmyTest < Minitest::Test
  ...
  def test_output_contain_response
    r = Wayarmy::Main.new()
    stub_request(:any, "https://gitlab.com").to_return(
      :status => 302
    )

    assert_output(/Response code: 302/) { r.start }
    assert_output(/Response Time/) { r.start }
    assert_output(/Response Time average:/) { r.start }
  end

  def test_response_time_will_appear_6_times
    $stdout = StringIO.new
    @r.start
